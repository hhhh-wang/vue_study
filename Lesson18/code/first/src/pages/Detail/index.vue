...
<!--          <a href="###">{{ categoryView.category3Name }}</a>-->
      ...
      <div class="mainCon">
        <div class="previewWrap">
          <Zoom :skuDefaultImg="currentImage" />

          <ImageList
              :images="skuInfo.skuImageList"
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="attr in spuSaleAttrList" :key="attr.id">
                ...
                <dd v-for="value in attr.spuSaleAttrValueList" :key="value.id" :changepirce="value.priceChange">{{ value.saleAttrValueName }}</dd>
              </dl>
            </div>
          </div>
        </div>
          ...
          <div class="tab-content">
            <div id="one" class="tab-pane active">
              <ul class="goods-intro">
                <li v-for="(attr, index) in skuInfo.skuAttrValueList" :key="index">
...
export default {
  ...
  computed: {
    ...mapState("product", {
      // 详情
      skuInfo: state => state.productInfo.skuInfo || {},
      // 配置
      spuSaleAttrList: state => state.productInfo.spuSaleAttrList || [],
      // 类别，面包屑导航
      categoryView: state => state.productInfo.categoryView || {}
    }),
  },
  data() {
    return {
      currentImage: '', // 初始化为空字符串
    };
  },
  methods: {
    handleImageSelected(imageUrl) {
      this.currentImage = imageUrl; // 更新当前显示的图像
    }
  },
  ...
  mounted() {
    this.$store.dispatch("product/getProductInfoByIdAsync", this.$route.params.id)
        .then(() => {
          // 等待商品信息加载完成后设置 currentImage
          this.currentImage = this.skuInfo.skuDefaultImg; // 确保在数据加载后设置
        });
  }
}
</script>
