<template>
    <div class="todo-container">
        <div class="todo-wrap">
            ...
            <ul class="todo-main">
                <li @mouseenter="activeId=item.id" @mouseleave="activeId=0" v-for="(item,index) in state.taskList"
                    :key="item.id">
                    <label>
                        <input @change="item.isChecked=!item.isChecked" :checked="item.isChecked" type="checkbox"/>
                        <span>{{ item.title }}</span>
                    </label>
                    <button class="btn btn-danger" @click="delTaskListByIndex(item.id)" v-show="item.id === activeId">
                        删除
                    </button>
                </li>
            </ul>
            <div class="todo-footer">
                ...
                <span>
              <span>已完成{{ state.taskList.filter(item => item.isChecked).length }}</span> / 全部{{ state.taskList.length }}
            </span>
                <button class="btn btn-danger" @click="clearAllIsChecked"
                        v-show="state.taskList.filter(item=>item.isChecked).length>0">清除已完成任务
                </button>
            </div>
        </div>
    </div>
...
const changeIsAll = function () {
    // isAll当前是否全选，再取反
    let a = !isAll();
    state.taskList = state.taskList.map((item:TTaskInfo) => {
        item.isChecked = a;
        return item;
    })
}
