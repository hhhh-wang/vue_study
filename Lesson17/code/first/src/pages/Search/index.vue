    ...
    <div class="py-container">
      ...
      <div class="bread" v-show="isSelector">
        ...
        <ul class="fl sui-tag">
          ...
          <li v-if="$route.query.trademark" class="with-x">
            {{ getTmName }}<i @click="moveTrademark">×</i>
          </li>
      ...
      <div class="details clearfix">
        ...
        <Pagination :continue="5"
                    ...
                    :pageSize="$store.state.product.searchProductResult.pageSize">
        </Pagination>
...
// 移除属性
    ...
    async changePageNo(pageNo) {
      // 更新 Vuex 中的 pageNo
      await this.$store.commit('product/SAVE_SEARCH_PRODUCT_RESULT', {
        ...this.$store.state.product.searchProductResult,
        pageNo
      })

      // 获取新的搜索结果
      await this.$store.dispatch('product/postProductListAsync', {
        pageNo: this.$store.state.product.searchProductResult.pageNo,
        pageSize: this.$store.state.product.searchProductResult.pageSize
      })
    }
  },
  watch: {
  ...
  mounted() {
    console.log(this.$route.query, JSON.stringify(this.$route.query) === "{}", Object.keys(this.$route.query).length === 0);
  },
  computed: {
    isSelector() {
    ...
    getTmName() {
      // trademark格式为 "tmId:tmName"，我们只需要显示tmName
      return this.$route.query.trademark?.split(':')[1] || '';
    },
    upOrDown(){
